...
lastReviewedNote: "Updated for Worker Issue #177 and Database #316: canonical v3 is one unified coalesced issue set with compact source-level root impact, frozen-graph witness reconstruction, and staged seal-before-upload publication."
related:
  - AGENTS.md
  - .docpact/config.yaml
  - docs/lca-api-contract.md
...
- `closure-bundle-v3.json`: requested bindings, scan and resolution evidence, unified issue summary, and a logical hash/count/path reference to the single compressed TIDAS stream; historical v1 bundles remain readable for certified package verification;
- `manifest.json`: `lcia.scope-closure-issue-manifest.v3`, binding the closure check, byte-exact TIDAS logical issue-stream SHA-256/count/bytes, unified issue and relation counts, zero expanded affected-root rows, graph/root-impact cardinalities and hashes, partition/sample limits, compatibility rules, and sorted artifact metadata;
