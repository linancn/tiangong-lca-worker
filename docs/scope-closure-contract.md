...
lastReviewedNote: "Updated for Issue #172 on the merged #174 baseline and Database #309 final contract commit 837948a: measured topology watermarks feed deterministic partitions whose manifest proves bounded full-result reconstruction, with role-tagged publication, trusted seven-day expiry, and retry-safe GC."
related:
  - AGENTS.md
  - .docpact/config.yaml
  - docs/lca-api-contract.md
...
- `manifest.json`: `lcia.scope-closure-issue-manifest.v2`, binding the closure check, the byte-exact TIDAS logical issue-stream SHA-256/count, exact relation counts, the SHA-256 of each complete reconstructed relation stream, partition limits, sample limits, and sorted partition metadata;
...
The manifest plus its listed compressed partitions is the only complete machine-result representation; Worker does not publish a second monolithic issue JSONL copy. A bounded streaming reconstruction reads the manifest in sorted path order, requires exact membership and contiguous relative partition identities, verifies every compressed and uncompressed checksum/size/count/boundary, enforces deterministic relation order across partition boundaries, and recomputes the manifest's global relation counts and stream hashes without materializing a partition or complete relation in memory. Every report artifact is published with one Database Engine #309 role: `closure_bundle`, `complete_machine_result`, or `closure_report`. Its database row records `ready` lifecycle state, the actual configured storage bucket and object path, SHA-256, byte size, content type, and `expires_at` calculated from trusted database time at publication plus seven days. The bundle metadata binds the preallocated manifest-row ID as `completeMachineResultArtifactId`, allowing the Database certificate guard to derive the evidence deadline.
...
The actor-bound download projection is `get_lcia_scope_closure_report_download(uuid,text)`. It accepts only `closure_report_xlsx` and `closure_issue_manifest` and returns the exact 11-field public descriptor: artifact ID/role/state, semantic filename, format, media type, size, checksum, expiry, bucket, and object path. Database Engine PR #309 final contract commit `7d40ab62fcf26de42f2c9b4aeb33acd47caa5f20` also keeps the temporary one-argument overload forwarding to `closure_report_xlsx`, adds two fixed-order locator-free artifact summaries to the owner check read, and owns the DB-first publication write set plus fenced GC preview/renew/reconcile RPCs. Its rollout-safe reconcile projection returns the persisted `cleanup_pending` state and current token/lease from a command after the claim update. Database maps public selectors to linked coarse-role rows; Worker does not expose or synthesize a separate download API. This delivery directly exposes only XLSX and the manifest. The manifest still identifies the complete partition set, but direct client retrieval of every partition requires a separately authorized cross-repository contract; Worker #172 does not invent an archive or broaden selectors to bypass that decision.
