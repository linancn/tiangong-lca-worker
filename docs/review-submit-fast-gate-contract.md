...
`source-reference-policy.v4` 做 role classification 和 artifact-purpose action。Process 数值轴只来自
...
reference 的 malformed UUID/version 使用稳定 `source_reference_invalid`。Administrative-support
reference 在 review-submit 中尽力读取，空占位、missing target 或 missing exact version 只进入
bounded provenance evidence；Certificate Closure 仍严格校验。`referenceToDigitalFile`
