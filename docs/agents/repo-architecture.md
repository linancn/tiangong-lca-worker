...
`crates/solver-worker/src/tidas_process_semantics.rs` owns shared target-aware TIDAS exchange allocation semantics. Object/array allocations select the current quantitative-reference target, interpret TIDAS `Perc` as percentage divided by `100`, treat a missing target in an otherwise closed vector as sparse zero, treat an undeclared allocation as `1`, and fail closed for invalid declared vectors. Explicit-zero and sparse-zero inputs are excluded from request-root provider closure, provider-gap diagnostics, and `A` writes; zero attributed elementary exchanges are also excluded from `B` and LCIA direction/factor-coverage evidence. Snapshot build config records `allocation_semantics_version = tidas-quantitative-reference-v1`; because the config participates in source fingerprinting, old-semantics snapshots are not reused. This does not change coverage payload shape, so coverage remains `snapshot_coverage.v2`.
