...
The process-column contract is one complete TIDAS Process revision per `quantitativeReference.referenceToReferenceFlow` and per snapshot matrix column. The referenced exchange may be any resolved Product, Waste, or Elementary `Input`/`Output`, as allowed by ILCD, and its calculation amount must be finite and positive. The reference exchange normalizes its process column and is excluded from provider demand. Provider eligibility is flow-kind directional: Product `Output` and Waste `Input` references may provide technosphere flows; Elementary references never do. Non-reference co-product exchanges remain rejected provider evidence; they do not create derived processes or matrix columns. When a co-product such as `B` must participate in calculation, upstream must publish another complete, independent Process whose quantitative reference is `B`.

`crates/solver-worker/src/tidas_process_semantics.rs` owns shared target-aware TIDAS exchange allocation semantics. Object/array allocations select the current quantitative-reference target, interpret TIDAS `Perc` as percentage divided by `100`, treat a missing target in an otherwise closed vector as sparse zero, treat an undeclared allocation as `1`, and fail closed for invalid declared vectors. It exposes only two bounded legacy fallbacks: a scalar `allocations.allocation = {}` is treated as undeclared, and one targetless full `100` / exact legacy `"100%"` entry is inferred for the reference only when the Process has exactly one physical `Output` whose sole valid internal ID equals the quantitative reference. Empty arrays, `[{}]`, multiple-output or multiple-entry targetless declarations, non-full targetless fractions, invalid Output IDs, and reference mismatches remain fail-closed. Explicit-zero and sparse-zero inputs, plus every quantitative-reference exchange, are excluded from provider demand and `A` writes; flow-kind directional provider eligibility is evaluated separately, and zero attributed elementary exchanges are excluded from `B` and LCIA direction/factor-coverage evidence. Snapshot build config records `allocation_semantics_version = tidas-quantitative-reference-v4`; because the config participates in source fingerprinting, snapshots built without the current ILCD reference and flow-kind directional-provider semantics are not reused. Coverage remains `snapshot_coverage.v2` with additive default-zero counters `legacy_empty_allocation_as_undeclared_count` and `legacy_single_output_target_inferred_count`, which make the compatibility paths observable without broadening them.
...
`crates/solver-worker/src/calculation_bundle.rs` owns canonical `tiangong.calculation-bundle.v1` generation. Snapshot compilation stores an additive exact `release_evidence` projection beside the sparse payload: quantitative references, source inventory identities, direct directional biosphere exchanges, resolved technosphere edges with both exchange internal IDs and provider weights, and the frozen canonical TIDAS source-document closure. The closure starts from the exact Process and Flow revisions selected by the snapshot plus the reviewed LCIA method identities, then resolves Contact, Flow Property, Source, Unit Group, and LCIA Method references by exact UUID/version; omitted support versions select the highest fixed-width ILCD version. The known reviewed method/artifact-locator alias remains a storage lookup only, while the TIDAS method UUID remains the released identity. New all-unit solves temporarily retain `x` only inside one fixed 256-process artifact chunk, derive directional LCI from those exact direct exchanges, retain H for the existing query view, and upload content-addressed sidecars before the manifest. Older snapshots without the exact release projection or source closure must be rebuilt and are never guessed from A/B or reconstructed from mutable database state during solve.
