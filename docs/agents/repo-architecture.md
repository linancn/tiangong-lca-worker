...
lastReviewedNote: "Reviewed for Worker Issue #209: snapshot-retention audit writes join the existing direct-PostgreSQL exact-private boundary without changing Edge or frontend responsibilities."
related:
  - ../../AGENTS.md
  - ../../.docpact/config.yaml
  - ./repo-validation.md
...
The main solver worker has two queue backends. The default `SOLVER_QUEUE_BACKEND=worker-jobs` path claims logical `worker_jobs` rows through `public.worker_claim_jobs` from `worker_queue=solver`, maps `job_kind=lca.*`, `job_kind=lcia_result.package_build`, and `job_kind=lcia.scope_closure_check` payloads back to internal `JobPayload` variants, heartbeats `phase/progress`, and records lease-fenced terminal results. Ordinary solve jobs link LCA domain rows and use `worker_record_job_result`; scope closure uses its V2 result or reuse-finalizer RPC because that same transaction persists issue provenance, evidence, certificate state, and the terminal Worker result. LCIA result package builds use `lca_results` plus `lca_latest_all_unit_results` as Worker-produced artifacts and then mark `lcia_result_packages` preview-ready through the database service-role command. Optional `lca_jobs` rows are best-effort compatibility only for ordinary LCA jobs; production worker_jobs paths must run when `public.lca_jobs` is absent. The `SOLVER_QUEUE_BACKEND=pgmq` path is legacy compatibility/debug only, consumes `pgmq` messages from `PGMQ_QUEUE`, and requires the explicit `ALLOW_LEGACY_JOB_TABLE_BACKEND=true` / `--allow-legacy-job-table-backend` guard.

Expand schema note: claim, heartbeat, enqueue, cancel, read, and terminal-result calls remain stable `public.worker_*` service RPCs. Every Worker direct read and the build-slot lease-fenced update use the schema-qualified `private.worker_jobs` contract; direct report metadata uses `private.worker_job_artifacts`. During Expand these private relations may be security-invoker compatibility views over public physical storage. A later gated Contract migration may swap physical storage without changing Worker DTOs, logical `worker_jobs` result references, or the stable public RPC surface. `public.worker_job_domain_refs` remains a cross-domain `UNION ALL` projection view and is not a private control-plane relation.
...
- The worker uses a main DB pool plus an optional queue-only DB pool. The main pool is configured through `DATABASE_URL` / `CONN`, `DB_MAX_CONNECTIONS`, `DB_MIN_CONNECTIONS`, and `DB_ACQUIRE_TIMEOUT_SECONDS`; it must use a dedicated non-superuser, non-BYPASSRLS Worker login on a session/direct connection or session pooler. The queue-only pool is configured through `QUEUE_DATABASE_URL` / `QUEUE_CONN`, `QUEUE_DB_MAX_CONNECTIONS`, `QUEUE_DB_MIN_CONNECTIONS`, and `QUEUE_DB_ACQUIRE_TIMEOUT_SECONDS`; if no queue URL is set it reuses the main pool. Solver scope closure additionally owns one temporary `DOCUMENT_VALIDATION_DATABASE_URL` family pool (default `2/0` max/min) whose application source references only Database #409 private evidence lookup/record. The inherited `lca_worker_runtime` role also retains its seven separately registered result-GC routines behind their own disabled/token gates; this family module never references them. Startup performs an exact nine-signature private EXECUTE census, so any unregistered private grant fails closed. The pool is mandatory and fail closed for solver-worker, never falls back, and is not constructed by package-worker or review-submit binaries. Startup pins PG17, a no-ownership/no-DDL restricted LOGIN, its exact `lca_worker_runtime` membership edge, the two evidence routine fingerprints/comments/ACLs with no same-name overload, denial of public wrappers and the physical table, and empty success envelopes without logging identity or target locators. The restricted LOGIN cannot read Supabase's migration ledger; the exact deployed migration head is operator evidence, while runtime pins the least-privilege Database #409 routine contract itself.
...
- Snapshot canonical persistence is a direct-PostgreSQL family boundary: all active Worker code, diagnostics helpers, lifecycle fixtures, and executable SQL under `docs/sql` address `private.lca_active_snapshots`, `private.lca_network_snapshots`, and `private.lca_snapshot_artifacts` explicitly. Snapshot-retention audit DML likewise addresses only `private.lca_snapshot_gc_runs` and `private.lca_snapshot_gc_run_items`. The Expand-era `public` views are not a fallback for either family. `scripts/qualify_snapshot_private_cutover.py` provides a pinned, scoped static diagnostic with quoted/comment/`ONLY` evasion fixtures for all five relations; it is not by itself final consumer-zero proof. Live proof never accepts an arbitrary database URL: it clones exact Database `86ba7ee2c33e45df8008117a2dec3ee4deedc32c` / migration `20260802091342` into a runner-owned temporary workdir, creates a unique Supabase project and ports, and destroys that project, its no-backup volumes/networks, and workdir after the run. Only this owned stack may be reset. The runtime evidence covers the dedicated-login ACL, twelve browser-role denials, search-path poisoning resistance, three upsert families, failure atomicity, concurrency, full-row façade parity, and zero qualification residue. Each TIDAS failure/success lifecycle performs actual exact-prefix Storage cleanup while a real sibling-prefix object is retained byte/hash-identically and then deleted by exact key; it never deletes the bucket wholesale. Database readback explicitly distinguishes count-only comparison across 91 discovered relations from full-content before/after hashes for the nine critical users, sessions, package, and private snapshot relations.
...
- The snapshot private-cutover scanner's reviewed active-source inventory is 103 files after adding the isolated document-validation DB module, source-commit build metadata, and qualification test; it enforces public/unqualified consumer zero for the three canonical snapshot relations and two snapshot-retention audit relations.
