...
| Supabase consumer manifest or any scanned consumer source | `python3 scripts/check_supabase_consumer_manifest.py`; `PYTHONPATH=scripts python3 -m unittest scripts/test_supabase_consumer_manifest.py`; `make check` | database-engine independently reads exact bytes from the immutable Worker commit and runs the real joint Supabase contract suite | The Worker artifact is candidate/non-authorizing until the database-owned freeze receipt accepts exact bytes and joint behavior passes. |
