...
lastReviewedNote: "Updated for Issue #172 on the merged #174 baseline and Database #309 final contract commit 837948a with two-run production-scale RSS/determinism proof, bounded manifest reconstruction, strict read/download/GC fixtures, and fresh-process detail-cleanup-without-redelete coverage."
related:
  - ../../AGENTS.md
  - ../../.docpact/config.yaml
...
| certificate-grade scope closure / package binding | `cargo test -p solver-worker scope_closure`; `cargo test -p solver-worker maps_scope_closure_payload_from_database_envelope`; `cargo test -p solver-worker package_closure_binding_is_all_or_none_and_result_ref_preserves_check_id`; `cargo check -p solver-worker --all-targets`; hard Clippy and format gates | run `cargo test --release -p solver-worker qualified_reference_graph_completes_within_five_minutes -- --ignored --exact`, `cargo test --release -p solver-worker qualified_million_event_spool_stays_within_fixed_runs -- --ignored --exact`, and the exact release test binary's ignored `scope_closure::tests::qualified_streaming_issue_merge_report_capacity` under an external process-tree RSS recorder. The streaming capacity test accepts `SCOPE_CLOSURE_REAL_PACKAGE_DIR`, `SCOPE_CLOSURE_REAL_ISSUE_SPOOL`, `SCOPE_CLOSURE_PRODUCTION_RAW_EVENTS`, `SCOPE_CLOSURE_PRODUCTION_RELATIONS`, and `SCOPE_CLOSURE_CAPACITY_OUTPUT`. It must inject real-width, high-unique-key root fan-out large enough for the derived affected-root run to exceed 2 GiB, then reconstruct every manifest-listed zstd partition through a bounded line reader, proving exact membership, contiguous relative identities, compressed/uncompressed checksum and count metadata, deterministic cross-partition order, and global relation counts/hashes without applying the raw-validator cap. The admission regression must model 5,608 global roots, about seven actually reachable roots per event, 516,313 events, and about 3.2 million relations on a 24 GiB-class volume; a separate test must prove a genuinely insufficient incremental run/merge watermark fails with `scope_closure_relation_temp_space_low`. Compare 16/32/64 MiB partition candidates at the same 25,000-record threshold, and run `qualified_streaming_affected_root_partition_scale` at 1×/2×/5×/10× with an external RSS recorder. Run `TIDAS_BIN=<release-bin> TIDAS_EXPECTED_VERSION=<version> cargo test -p solver-worker tidas_cli::tests::release_binary_completes_version_and_protocol_handshake -- --ignored`; with isolated non-production DB/S3 and matching database-engine migrations, run one fresh and one concurrently deduplicated closure; verify current-release snapshot hashes, live-drift/live-only rejection, exact-version union/cycle traversal, byte-exact non-empty TIDAS issue hashing, TIDAS cache keys and final event, manifest plus NDJSON+zstd partition hashes/counts/boundaries, bounded XLSX/RPC samples, upload heartbeat/cancellation/cleanup, target-specific reuse report, certificate binding, and unchanged package numerical artifacts | Before server execution, use the local real-package capacity fixture and confirm document/reference spools plus compact graph stay within the process-tree RSS budget, 32 MiB sort runs, 8 MiB cache writes, role-specific raw spool caps, observed-raw initial temp admission, measured-topology incremental watermarks with the fixed reserve, cleanup, deterministic V1 and partition bytes across two complete runs, worksheet/member/total artifact preflight, post-merge/report RSS and Linux cgroup anon/file/current/peak measurements, exact recovered relation counts and hashes, and generated high-unique 1×/2×/5×/10× affected-root scale behavior. Never use production mutation. Keep large package inputs and generated output outside git. Keep `docs/scope-closure-contract.md`, `docs/lca-api-contract.md`, and `docs/tidas-package-contract.md` aligned with the database RPC signatures and TIDAS public protocol. A mock-only traversal is not sufficient integration proof. |
