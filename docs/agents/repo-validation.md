...
| snapshot-builder or provider-matching behavior | baseline gates plus `./scripts/build_snapshot_from_ilcd.sh` when safe; for quantitative-reference direction changes, prove both incoming normalization/no-demand behavior and unchanged outgoing provider eligibility | run provider-link diagnostics export helpers when the task changes matching logic | Keep `docs/provider-linking.md` and the implicit regional supply mix docs aligned with runtime semantics. Snapshot and provider diagnostics often need task-specific proof. |
