...
lastReviewedNote: "Issue #148 adds issue-heavy graph-finalization coverage and requires phase-timing review before capacity rollout."
related:
  - ../../AGENTS.md
  - ../../.docpact/config.yaml
...
4. Verify that `allocation_semantics_version`, `link_semantics_version`, `technosphere_boundary_policy`, `flow_identity_policy`, and `source_closure_policy` participate in snapshot and review identity/reuse decisions. Confirm that changed semantics produce a newly built snapshot rather than reusing the baseline.
