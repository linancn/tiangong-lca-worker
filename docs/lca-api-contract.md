...
solver worker 可通过 `SOLVER_QUEUE_BACKEND=worker-jobs` 或 `--queue-backend worker-jobs` 切到 `public.worker_jobs` claim 模式。该模式只领取 `worker_queue=solver` 的 LCA solve jobs：
...
On failure, the worker updates legacy `lca_jobs` / `lca_result_cache` consistently and records `worker_jobs.status=failed` with `error_code=solver_worker_job_failed`.
