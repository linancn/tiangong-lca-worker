...
Build config 记录 `allocation_semantics_version = tidas-reference-allocation-v3`、`link_semantics_version = signed-flow-balance-v1`、`technosphere_boundary_policy`、`flow_identity_policy = exact-flow-version-reference-unit-v2` 与 `source_closure_policy = selected-lcia-factor-flow-support-v1`；所有字段进入 source/review fingerprint，solver contract 也显式记录 source-closure policy。Flow identity v2 只查询和编译最终 process closure exchange 实际引用的 exact Flow identities；source-closure v1 只为 selected LCIA-factor-only Elementary Flow 增加 support evidence，不扩大稀疏矩阵轴。旧 exchange-only closure snapshot 因 policy/fingerprint 不同而不可复用。
