...
lastReviewedNote: "Updated for Worker Issue #221: source-reference policy v4 and bounded blocker diagnostics preserve the existing job/status contract."
related:
  - AGENTS.md
  - .docpact/config.yaml
  - docs/provider-linking.md
...
- 普通 fresh snapshot 不持久化完整 `CompiledGraph`。`snapshot-hdf5:v1` 只保存 numerical payload/config/coverage，并通过 URL、SHA-256、byte size、format 与 content type descriptor 绑定独立的 `snapshot-release-evidence-json-zstd:v1` sidecar；sidecar 只保存 Calculation Bundle 所需的 `CompiledReleaseEvidence`（包括 `source_datasets`），以固定内存流式编码并通过 bounded multipart path 上传。常规 solve 只读取 HDF5；只有 Calculation Bundle materialization 才下载、校验并解码 release-evidence sidecar。历史内嵌 `compiled_graph.release_evidence` 的 HDF5 继续兼容读取。review-submit baseline/overlay 是独立的 gate artifact，仍按 review contract 保存 gate 所需 graph，不能被普通计算 artifact 复用。source closure 从本次 snapshot 精确选择的 Process/inventory Flow revision 和已审 LCIA Method identity 出发；另外一次扫描 selected Method 的全部 factor Flow reference（包括 zero factor），按 UUID/version 去重后 bounded-batch 解析为 Elementary Flow support root，再递归解析 Contact、Flow Property、Source、Unit Group 与 LCIA Method reference。显式版本只允许 exact match，省略的 support version 只确定一次并冻结，缺失、歧义、无效 UUID/version、非 Elementary factor target 或同 identity/version 内容漂移均 fail closed。这个 source-evidence 扩展不改变 B/C axis、compiler IR 或 provider lookup；support documents 不在 solve 时重新查询。
