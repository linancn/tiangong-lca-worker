...
下载 descriptor 由 Database 的 actor-bound `get_lcia_scope_closure_report_download(uuid,text)` 投影；selector 只能是 `closure_report_xlsx` 或 `closure_issue_manifest`。成功响应固定为 11 个字段：`artifactId`、public `artifactRole`、`artifactState`、semantic `filename`、`format`、`mediaType`、`size`、`checksumSha256`、`artifactExpiresAt`、`bucket`、`objectPath`。Database Engine PR #309 final contract commit `837948a789339376daa2c71c1b7fa3f0143ef211` 新增 owner read 的两个固定顺序、无 locator summaries，并保留临时单参数 overload 转发 XLSX；这两个 rollout compatibility 改动不改变 Worker GC payload。本批 UI/API 只直接下载 XLSX 和 manifest；manifest 列出的 partitions 共同构成完整可恢复结果，但逐 partition 的客户端下载授权需要另一个 tracked cross-repo contract。Worker 不新增 archive、selector、actor ACL 或 public DTO 映射来绕过该边界。
