...
- `provider_evidence`: 每条实际 technosphere demand edge 的 consumer、flow、candidate providers、candidate directional-reference status / eligibility、resolution strategy、failure reason、allocation weights、ambiguity 和 confidence；被判定为 non-reference 的 rejected candidate 也必须保留，供调用方解释 provider 缺口。
...
| `provider_closure_reference_provider_missing` | 某条实际 technosphere demand decision 的 `failure_reason = rejected_non_reference_only`：存在同 flow 的 directional candidate，但它们都不是各自 Process 的 quantitative reference | 发布或引入一个以该 flow 为 quantitative reference、方向符合 Product Output / Waste Input 语义的完整 Process 后重跑 | 否 |
...
Snapshot build config 使用 `allocation_semantics_version = tidas-quantitative-reference-v4`，且该字段进入 source fingerprint，所以 readiness 不会复用缺少 ILCD 任意 Input/Output reference 与 flow-kind directional provider 语义的旧 snapshot。`snapshot_coverage.v2` 和 `matrix_readiness_report.v1` 均保持现有 schema version；两个计数字段为 additive/default-zero，旧 artifact 缺少时按 `0` 读取。

合法的 quantitative reference 可以是 Product、Waste 或 Elementary flow 的 Input/Output。Reference exchange 只归一化 process column，并从 provider demand 中排除；Product Output 与 Waste Input 可具有 provider eligibility，Elementary 永不进入 technosphere provider matching。flow metadata 不可解析、无效方向、重复匹配或非正参考量仍在 snapshot build 阶段 fail closed。

`provider_closure_reference_provider_missing` 只针对实际 technosphere demand decision，不会因为 Process 中一个没有被 demand 的 co-product 本身而触发。其 `details.examples[]` 必须给出 consumer index / ID / version / name、`flow_id`、candidate provider index / ID / process name、candidate exchange internal ID、direction、是否为 reference、normalized amount、allocation state 与 eligibility。它可以与通用的 `provider_closure_unmatched` 同时出现：前者解释“为什么没有合法 provider”，后者仍表达 coverage policy 失败。
