...
- `compiled_graph`（可选）：fresh build 时包含 `reference_ports`、`balance_resolutions`、`unresolved_balances`，以及兼容的 provider decision projection。没有该字段时仍可验证 coverage/compute，但逐边解释证据会为空。
...
- `provider_evidence`: 保留给旧调用方的候选/路由兼容投影；新调用方应优先消费 balance evidence。
